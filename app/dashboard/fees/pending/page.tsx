...
export default function PendingFees() {
  ...
  const fetchPendingFees = async () => {
    try {
      // Auto-generate due fees before fetching the list
      await fetch('/api/fees/generate', { method: 'POST' });
      ...
      pendingFees.forEach(fee => {
        if (!summaryMap.has(fee.student_id)) {
          summaryMap.set(fee.student_id, {
            ...
            student_seat_number: fee.student_seat_number,
      ...
      const sortedStudents = Array.from(summaryMap.values()).sort((a, b) =>
        (a.student_seat_number || '').localeCompare(b.student_seat_number || '', undefined, { numeric: true })
      );
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Seat No</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {pendingStudents.map((student) => (
                  <tr key={student.student_id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-sm font-medium text-indigo-600">{student.student_seat_number || 'N/A'}</td>
